...
// 使用 CommandLineToArgvW 获取正确解析的 UTF-8 参数列表
static std::vector<std::string> get_utf8_args() {
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        if (len > 0) {
            ...
            WideCharToMultiByte(CP_UTF8, 0, argvW[i], -1, &s[0], len, nullptr, nullptr);
            args.push_back(s);
        }
    }
    LocalFree(argvW);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR /*lpCmdLine*/, int nCmdShow) {
    ...
    if (args.size() > 1) {
        ...
        for (size_t i = 1; i < args.size(); ++i) {
            ...
            if (tok == "-o" || tok == "--output") {
                if (i + 1 < args.size()) output_dir = args[++i];
            } else if (tok == "--validate-only") {
                validate_only = true;
            } else if (tok == "--dump-text") {
                dump_text = true;
            } else if (tok == "--no-summary") {
                no_summary = true;
            } else if (tok == "--summary-only") {
                summary_only = true;
            } else if (tok == "--enable-ocr") {
                enable_ocr = true;
